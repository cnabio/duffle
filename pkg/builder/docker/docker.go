...
func (b Builder) BuildComponents(ctx context.Context, app *builder.AppContext, out chan<- *builder.Summary) (err error) {
	...
	go func() {
		...
		for _, c := range app.Ctx.Components {
			...
			go func(dc *Component) {
				...
				if err != nil {
					errc <- fmt.Errorf("error building component %v: %v", dc.Name, err)
					return
				}
