...
func (r *KeyRing) SavePublic(filepath string, clobber bool, armored bool) error {
	if !clobber {
		if _, err := os.Stat(filepath); err == nil {
			return errors.New("keyring file exists")
		}
	...
	// Write to a buffer so we don't nuke a keychain.
	temp := bytes.NewBuffer(nil)
	if err := r.SavePublicTo(temp, armored); err != nil {
		return err
	}
	_, err = io.Copy(f, temp)
	return err
...
func (r *KeyRing) SavePublicTo(w io.Writer, armored bool) error {
	for _, e := range r.entities {
		if armored {
			pubkeyWriter, err := armor.Encode(w, openpgp.PublicKeyType, nil)
			if err != nil {
				return err
			}
			w = pubkeyWriter
...
func LoadKeyRing(path string, armored bool) (*KeyRing, error) {
	// TODO: Should we create a default passphrase fetcher?
	return LoadKeyRingFetcher(path, nil, armored)
}
...
func LoadKeyRings(armored bool, paths ...string) (*KeyRing, error) {
	if len(paths) == 0 {
		return &KeyRing{}, errors.New("no keyrings provided")
	}
	baseRing, err := LoadKeyRing(paths[0], armored)
	if err != nil {
		return baseRing, err
	}
	for i := 1; i < len(paths); i++ {
		ring, err := LoadKeyRingFetcher(paths[i], baseRing.PassphraseFetcher, armored)
		if err != nil {
			return baseRing, err
		}
...
func LoadKeyRingFetcher(path string, fetcher PassphraseFetcher, armored bool) (*KeyRing, error) {
	var (
		entities openpgp.EntityList
		err      error
		keyFile  *os.File
