...
func relocateFileToFile(t *testing.T, bundle string, expectedErr error, archiveDirStub func(archiveDir string)) {
	...
	cmd := &relocateCmd{
		...
		imageStoreBuilder: &mockImageStoreBuilder{
				...
				return &mockImageStore{
					pushStub: func(dig image.Digest, src image.Name, dst image.Name) error {
						...
						digests := map[string]pair{
							"docker.io/" + originalInvocationImageName: pair{"", relocatedInvocationImageName},
							"docker.io/" + originalImageNameA: pair{imageDigestA, relocatedImageNameA},
							"docker.io/" + originalImageNameB: pair{originalImageDigestB, relocatedImageNameB},
						}
...
type mockImageStoreBuilder struct {
	archiveDirStub func(string)
	logsStub       func(io.Writer) imagestore.Builder
	buildStub      func() (imagestore.Store, error)
}

func (b *mockImageStoreBuilder) ArchiveDir(archiveDir string) imagestore.Builder {
	b.archiveDirStub(archiveDir)
	return b
}

func (*mockImageStoreBuilder) Logs(io.Writer) imagestore.Builder {
	panic("implement me")
}

func (b *mockImageStoreBuilder) Build() (imagestore.Store, error) {
	return b.buildStub()
}
...
func (*mockImageStore) Add(img string) (contentDigest string, err error) {
	panic("implement me")
}
